...
const Blog = ({ id, heading, image, text, date, index }) => {
    return (
        <Link to={`/all-blogs/${id}`} className="bg-slate-700 bg-opacity-25 backdrop-blur-lg rounded-xl border-[1.5px] border-[#ffffff15] z-40">
            <motion.div
                initial={{ opacity: 0, translateY: 150 }}
                whileInView={{ opacity: 1, translateY: 0 }}
                transition={{ duration: 0.4, delay: index / 10 }}
                className="flex flex-col gap-5 p-4 rounded-xl text-[0.8rem] justify-center cursor-pointer"
            >
